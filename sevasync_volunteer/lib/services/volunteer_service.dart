...
class VolunteerService {
  ...
  // ── Messages ──────────────────────────────────────────────────────────────
  /// Get all admins this volunteer can message
  /// Tries role filter first; falls back to admin_id on own profile if RLS blocks it
  static Future<List<VolunteerProfile>> getAdmins() async {
    try {
      // First try: fetch all admin/super-admin profiles
      final rows = await _db.from('profiles').select()
          .inFilter('role', ['admin', 'super-admin'])
          .order('name');
      final admins = rows.map<VolunteerProfile>(VolunteerProfile.fromJson).toList();
      if (admins.isNotEmpty) return admins;
    } catch (_) {}

    // Fallback: get admin_id from own profile
    try {
  ...
  /// Get conversation between volunteer and a specific admin
  ...
  /// Get last message + unread count per admin
